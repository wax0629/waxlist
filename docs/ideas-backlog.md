...
|--------|------|--------------|-----------------|
| **P0** | **搜索机制强化**（需求 → Intent → query 族 → 过滤重排 → 短名单） | 否则与用户自己搜 YouTube 差异弱 | [search-strategy.md](./search-strategy.md) · **[spec-v0.2](./spec-v0.2.md)** |
| **P1** | **优化 UI**（策略/理解可视化 + 质感） | 可演示性与信任感 | **[spec-v0.2](./spec-v0.2.md)** |

### 1. 搜索机制强化

- **状态：** planned（已进 spec-v0.2）  
- **大概版本：** v0.2  
...
| 主题 | 文档 | 状态 |
|------|------|------|
| **搜索排名逻辑是什么** | [ranking-logic.md](./ranking-logic.md) | **已写**（加减分、制作人、LLM 边界） |
| 搜索策略总论 | [search-strategy.md](./search-strategy.md) | 已有 |
| UI 成品质感 | Phase 1 进行中 | 代码迭代 |
...
## 完整产品阶段（见 product-maturity）

| Phase | 含义 | 状态 |
|-------|------|------|
| 0 闭环验证 | 主路径能演示 | 基本完成 |
| **1 可对外 Beta（仅 Agent）** | 检索 + /chat UI + 持久化 + 部署 | **当前唯一主攻** |
| 2 主产品做透 | 账号、收藏、多源、观测 | 后续 |
| 3 地下精选认真做 | explore + 打分 + 弱连接（用户重视，不砍） | **后置，不挤 Phase 1** |
